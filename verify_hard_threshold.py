...
print("=" * 65)
print(" VISIONARYAI HARD 0.60 (60%) CONFIDENCE VERIFICATION SUITE")
print("=" * 65)

# 0. Pure boundary logic test: confidence >= 0.60 (not > 0.60)
print("\n0. Confidence Threshold Boundary Logic Tests (conf >= 0.60):")
boundary_cases = [(59.0, False), (59.9, False), (60.0, True), (60.1, True), (70.0, True), (90.0, True)]
for conf_pct, expected in boundary_cases:
    actual = (conf_pct / 100.0) >= 0.60
...
# 2. Test Direct Inference on Sample Products with Hard 0.60
print("\n2. Direct YOLO Inference Tests (conf=0.60):")
sample_products = ['trident.jpeg', 'donut.jpeg', 'pickers.jpeg', 'bahia.jpeg']
for prod in sample_products:
    p_path = os.path.join('assets', 'products', prod)
    if os.path.exists(p_path):
        results = model.predict(p_path, conf=0.60, verbose=False)[0]
        detected = []
...
print("\n3. Negative Tests (Hands, Face, Empty Backgrounds - conf=0.60):")
negatives = {
...
for name, img in negatives.items():
    results = model.predict(img, conf=0.60, verbose=False)[0]
    detected = []
    for box in results.boxes:
        cid = int(box.cls[0].item())
        conf = float(box.conf[0].item())
...
def test_api_detect(image_input, label, conf=0.60):
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=5) as resp:
            ...
            # Check if any detection leaked below 0.60
            leaks = [d for d in dets if float(d.get('confidence', 0)) < 0.60]
            if leaks:
                print(f"   [API Test] {label:<22} -> FAILED! Found detections below 0.60: {leaks}")
            else:
...
print("\n" + "=" * 65)
print(" ALL HARD 0.60 CONFIDENCE THRESHOLD CHECKS COMPLETED")
print("=" * 65)
